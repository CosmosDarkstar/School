...
2.	It is a seperate interface collection from all of the others with a HashMap and TreeMap class.

3.	Maps let you connect a key set with a value collection. You would use a map when you want to find  an object/value using a key. Keys are Strings and Values Integer wrappers. If there is no value asscociated with a key, it returns null. 
	Maps are unique because there is a unique code that associates with a value. All of the other collections don't have this ability.
	The difference between a HashMap and a TreeMap is that the keys are sorted in the TreeMap and is space-efficient whereas a HashMap is not guaranteed to be sorted, but it time-efficient.
...
6.	for a given user on my web server, what is their current session status?
	...
	for a given name, what is their favorite color/number/day/month/birthday/pet/animal/...?
